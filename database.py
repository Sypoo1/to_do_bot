...
conn = psycopg2.connect(database="to_do_bot", user="postgres",
                        password='justdoit8', host="localhost", port="5432")
...
def get_tasks():
        cursor.execute('select * from tasks where id NOT IN (SELECT id FROM hide WHERE id IS NOT NULL );')
        records = list(cursor.fetchall())
        return records

def get_tasks_boolean(bol):
    cursor.execute(f'select * from tasks where completed={bol} and id NOT IN (SELECT id FROM hide WHERE id IS NOT NULL );')

    records = list(cursor.fetchall())
    return records

def get_by_id(id):
    cursor.execute(f'select * from tasks where id={id} and id NOT IN (SELECT id FROM hide WHERE id IS NOT NULL );')
    records = list(cursor.fetchall())
    return records
